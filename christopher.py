...
LLAMA_NGL         = int(ENV.get("LLAMA_NGL") or str(_profile_default(CURRENT_MODEL_PROFILE, "default_ngl")))
LLAMA_THREADS     = int(ENV.get("LLAMA_THREADS") or "4")
LLAMA_CTX         = int(ENV.get("LLAMA_CTX") or str(_profile_default(CURRENT_MODEL_PROFILE, "default_ctx")))
...
def configure_model_runtime(profile: str, model_path: str = "", ngl: int | None = None, ctx: int | None = None):
    ...
    LLAMA_NGL = int(ngl if ngl is not None else (ENV.get("LLAMA_NGL") or str(_profile_default(profile, "default_ngl"))))
    LLAMA_CTX = int(ctx if ctx is not None else (ENV.get("LLAMA_CTX") or str(_profile_default(profile, "default_ctx"))))
